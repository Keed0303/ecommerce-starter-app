...
import { type AvailableModule, type BreadcrumbItem, type ModuleOrder, type PermissionGroup, type Role } from '@/types';
...
export default function EditRole({ role, permissions, modules }: EditRoleProps) {
    const { data, setData, put, processing, errors } = useForm({
        name: role.name || '',
        description: role.description || '',
        permissions: role.permissions?.map((p) => p.id) || [],
        moduleOrder: role.module_order || [],
    });

    const [moduleOrder, setModuleOrder] = useState<ModuleOrder[]>(role.module_order || []);

    // Initialize module order if empty
    useEffect(() => {
            ...
            <div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto p-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground">Update role details, permissions and module order</p>
                    </div>
                </div>
